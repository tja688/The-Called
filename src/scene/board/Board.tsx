...
function StagedCell({ index, ...props }: { index: number; id: Parameters<typeof Cell>[0]['id']; position: readonly [number, number, number] }) {
  ...
  useFrame((state) => {
    ...
    const progress = stageProgress(stageNow(battleKey, state.clock.elapsedTime), STAGE_INTRO.boardStart + index * STAGE_INTRO.boardStep, STAGE_INTRO.boardCellDuration)
