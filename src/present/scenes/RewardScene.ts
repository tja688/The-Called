...
import { drawHandCard, drawInspectPanel } from '../widgets'
...
export class RewardScene extends Scene {
  ...
  render(world: CanvasRenderingContext2D, ui: CanvasRenderingContext2D, text: TextLayer): void {
    ...
    text.draw('不自动进牌组。回地图后再编。指向卡牌看说明。', x + 20, y + 42, { size: 12, color: PAL.cream })
    let hoverDef = picked
    pool.forEach((id, i) => {
      ...
      if (!picked) {
        ...
        this.app.ui.hit(`rew-${id}`, { x: cx, y: cy, w: 72, h: 108 }, () => { /* inspect */ }, 2)
      }
      if (this.app.input.isHover(`rew-${id}`)) hoverDef = id
    })
    const show = hoverDef ?? pool[0]
    if (show) {
      const def = this.app.ask({ type: 'content.card', defId: show })
      drawInspectPanel(ui, text, { def }, x + 20, y + 180, w - 40, 64)
    }
