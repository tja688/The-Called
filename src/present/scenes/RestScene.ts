...
export class RestScene extends Scene {
  ...
  render(world: CanvasRenderingContext2D, ui: CanvasRenderingContext2D, text: TextLayer): void {
    ...
    text.draw('休息处', x + 20, y + 16, { size: 18, bold: true, color: PAL.lamp1 })
    text.draw('井水可以喝。也可以在这里把英雄练得更硬一点。', x + 20, y + 44, { size: 12, color: PAL.cream })
    this.app.ui.button('heal', { x: x + 20, y: y + 80, w: w - 40, h: 40 }, '休息 · 回复生命上限的 30%', () => {
      this.picked = true
      audio.sfx('click')
      this.app.send({ type: 'run.restPick', choice: 'heal' })
    }, { disabled: this.picked })
    this.app.ui.button('grow', { x: x + 20, y: y + 132, w: w - 40, h: 40 }, '锻炼 · 英雄基础点数 +2，生命上限 +2', () => {
      this.picked = true
